...
static void test_latest_buffer_fanout(void)
{
	...
	struct pw_buffer *published, *producer_owned, *returned;
	uint64_t sequence[2];
	uint32_t id[2];
	...
	producer_owned = pw_stream_dequeue_buffer(stream);
	spa_assert_se(producer_owned != NULL);
	...
	spa_assert_se(pw_stream_queue_buffer(stream, returned) == 0);
	spa_assert_se(pw_stream_try_dequeue_buffer_reusable(stream,
			&returned) == 0);
	spa_assert_se(returned == NULL);
	spa_assert_se(pw_stream_try_reclaim_buffer_latest(stream, &returned) == 1);
	spa_assert_se(returned == published);
	spa_assert_se(spa_io_buffers_latest_receive(&latest[0], &sequence[0],
			&id[0]) == -EPIPE);
	spa_assert_se(spa_io_buffers_latest_receive(&latest[1], &sequence[1],
	...
	spa_assert_se(pw_stream_return_buffer(stream, producer_owned) == 0);
